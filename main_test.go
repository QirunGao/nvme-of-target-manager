...
func TestStopDoesNotRemoveQoS(t *testing.T) {
	...
	paths := Paths{
		...
		Hosts:          filepath.Join(base, "hosts"),
		AllowedHosts:   filepath.Join(base, "allowed_hosts"),
		RDMACMDevice:   filepath.Join(base, "rdma_cm", "mlx5_0"),
		RDMACMPort:     filepath.Join(base, "rdma_cm", "mlx5_0", "ports", "1"),
		RDMADefaultTOS: filepath.Join(base, "rdma_cm", "mlx5_0", "ports", "1", "default_roce_tos"),
	}

	for _, dir := range []string{paths.Subsystems, paths.Subsystem, paths.Namespaces, paths.Namespace, paths.Ports, paths.Port, paths.PortSubsystems, paths.AllowedHosts} {
		if err := os.MkdirAll(dir, 0700); err != nil {
			t.Fatal(err)
		}
	}
	...
	writeFile(t, paths.RDMADefaultTOS, "106")
